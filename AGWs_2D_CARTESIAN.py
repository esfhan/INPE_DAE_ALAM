...
def ambiente(x2,y2):
    ...
    global rho_o,tn_o,r_g,nu_nn,lambda_c
...
#==================================MAIN=======================================#
#=============================================================================#    
#(wx,wy) são amplitudes da AGWs na direções (x,y) 
...
t_f=2.*t_o;nt=int(t_f/dt)+1;time=arange(0,0+nt*dt+dt,dt)                                  
wx_t=zeros((nt,ny));wy_t=zeros((nt,ny))
...
#    cs=sqrt(1.4*pn/rho);dt=0.5*dy/cs.max()
    ...
    if remainder(it,10)==0:
        ...
        xlabel('Temperatura $^o$K');ylabel('Altitude, km')
        
        fig=figure(2,figsize=(12,12),facecolor='w',edgecolor='k') 
        subplot(221)
        vm=wx.max()/5.
        pcolormesh(zoom(x2,2),zoom(y2,2),zoom(wx,2),cmap=cm.seismic,vmin=-vm,vmax=vm)
        title('Horizontal Amplitude $(W_x)$ of AGWs, m/s')
        subplot(222)
        pcolormesh(zoom(x2,2),zoom(y2,2),zoom(wy,2),cmap=cm.seismic,vmin=-vm,vmax=vm)
        title('Vertical Amplitude $(W_y)$ of AGWs, m/s')
        subplot(223)
        vm=drho.max()/5.
        pcolormesh(zoom(x2,2),zoom(y2,2),zoom(drho,2),cmap=cm.seismic,vmin=-vm,vmax=vm)
        title(r'$Density perturbation \delta rho/rho_o$, %')
        xlabel('Longitude/Latitude, km');ylabel('Altitude, km')
        subplot(224)
        vm=dtn.max()/5.
        pcolormesh(zoom(x2,2),zoom(y2,2),zoom(dtn,2),cmap=cm.seismic,vmin=-vm,vmax=vm)
        title('Temperature perturbation $\delta T/T_o$, %')
        xlabel('Longitude/Latitude, km');ylabel('Altitude, km')
        
        fig=figure(3,figsize=(12,12),facecolor='w',edgecolor='k') 
        subplot(121)
        vm=wx.max()/2.
        pcolormesh(time/60.,y2[0,:],wx_t.T,cmap=cm.seismic,vmin=-vm,vmax=vm)
        title('Amplitude horizontal $(W_x)$, m/s as x=0 km')
        xlabel('Time, minutos');ylabel('Altitude, km')
        subplot(122)
        vm=wy.max()/2.
        pcolormesh(time/60.,y2[0,:],wy_t.T,cmap=cm.seismic,vmin=-vm,vmax=vm)
        title('Amplitude vertical $(W_y)$, m/s as x=0 km')
        xlabel('Time, minutos');ylabel('Altitude, km')
        draw()
        pause(0.1)
        clf()
...
#        i_xo=int((nx+1)/2)
#        subplot(121)
#        plot(it+10*wx[i_xo,:]/abs(wx[i_xo,:]).max(),y2[i_xo,:])
#        subplot(122)
#        plot(it+10*wy[i_xo,:]/abs(wy[i_xo,:]).max(),y2[i_xo,:])
#        
#        draw()
#        pause(0.1)
