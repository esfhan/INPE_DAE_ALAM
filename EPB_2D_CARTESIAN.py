#ESTE CÓDIGO RESOLVE AS EQUAÇÕES DADA POR KHERANI ET AL (2016,10.1063/1.4945636)..
...
#==============================================================================        
    ...
    ep_x=gradient(-phi)[0]/(2.*dx2)                                              #campo eletrico de polarização
    ep_y=gradient(-phi)[1]/(2.*dy2)
...
#==============================================================================    
    ...
    a=gradient(n)[0]/(dx2*sum_gr(0,nx,n))
    b=gradient(n)[1]/(dy2*sum_gr(1,ny,n))
    s_x=vo_x*a/mu_p
    s_y=vo_y*b/mu_p
    ...
    for iter in range (11):                                                     #ITERATION LOOP PARA GAUSS-SEIDEL CONVERGENCE
        ...
        phi_x=a*gradient(phi)[0]/(2.*dx2)
        phi_y=b*gradient(phi)[1]/(2.*dy2)
        
        phi_2x=sum_gr(0,nx,phi)
        phi_2y=sum_gr(1,ny,phi)
        
        phi_x2=phi_2x/(2.*dx2**2.)
        phi_y2=phi_2y/(2.*dy2**2.)
